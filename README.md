...
Claude Code は `CLAUDE_CONFIG_DIR` 環境変数が設定されている場合、`~/.claude` の代わりにそのディレクトリを設定・会話ログの置き場所として使う。discord-agent-pilot は、検出した tmux ペイン内の `claude` プロセスの `/proc/<pid>/environ` からこの値を読み取り、セッションごとに正しい JSONL 探索ルートへ切り替える(未設定時は `claude.defaultConfigDir` にフォールバックする)。
...
- `hostPath` は `/proc/<pid>/environ` から取得したホスト側の生の値と文字列比較されるため、`~` は展開されない。絶対パスで指定すること。
